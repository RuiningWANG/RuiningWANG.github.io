...
from resnet_no_bn import MIL_model

#Put training and testing into GPU
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

model = MIL_model().to(device)
...
for epoch in range(num_epochs):
    ...
    print(f"Epochs: {epoch+1}/{num_epochs}, Loss_train: {train_loss}, loss_valid: {val_loss}")
